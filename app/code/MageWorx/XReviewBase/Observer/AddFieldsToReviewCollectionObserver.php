...
class AddFieldsToReviewCollectionObserver implements ObserverInterface
{
    /**
    ...
    public function execute(\Magento\Framework\Event\Observer $observer)
    {
        ...
        if ($collection instanceof \Magento\Review\Model\ResourceModel\Review\Collection) {
            $from = $collection->getSelect()->getPart(\Magento\Framework\DB\Select::FROM);
            if (!empty($from['detail'])) {
                $fields = [];
