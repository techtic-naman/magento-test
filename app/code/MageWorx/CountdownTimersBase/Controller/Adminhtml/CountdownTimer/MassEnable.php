...
class MassEnable extends \MageWorx\CountdownTimersBase\Controller\Adminhtml\CountdownTimer\MassAction
{
    /**
    ...
    {
        if ((int)$countdownTimer->getStatus() !== StatusOptions::ENABLE) {
            $table      = $this->resourceConnection->getTableName(CountdownTimer::COUNTDOWN_TIMER_TABLE);
            $connection = $this->resourceConnection->getConnection();
            $connection
                ->update(
                    $table,
                    [CountdownTimerInterface::STATUS => StatusOptions::ENABLE],
