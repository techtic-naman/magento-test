...
class Sendmail extends Action
{
     ...
     *
     * @return \Magento\Framework\Controller\Result\RedirectFactory|void|bool
     */
    public function execute()
    {
        ...
        if ($data['seller-id']) {
            ...
            $emailTemplateVariables['customerName'] = $seller->getName();
            $sellerEmail = $seller->getEmail();
            if (!isset($data['product-id'])) {
                $data['product-id'] = 0;
            } else {
                $emailTemplateVariables['productName'] = $this->_product->load(
                    $data['product-id']
                )->getName();
            }
            $emailTemplateVariables['query'] = $data['ask'];
            $emailTemplateVariables['customerSubject'] = $data['subject'];
            $senderInfo = [
                'name' => $buyerName,
                'email' => $buyerEmail,
            ];
