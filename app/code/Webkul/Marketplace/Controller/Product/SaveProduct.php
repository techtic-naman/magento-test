...
class SaveProduct
{
    ...
    public function saveProductData($sellerId, $wholedata)
    {
        ...
        if ($mageProductId) {
            $statusProduct = $helper->getIsProductEditApproval() ?
            ...
            if ($helper->getIsProductApproval() && !$helper->getIsProductEditApproval()) {
                ...
                foreach ($sellerProductColls as $sellerProductColl) {
                    $statusProduct = !$sellerProductColl->getIsApproved() ?
                    SellerProduct::STATUS_DISABLED : SellerProduct::STATUS_ENABLED;
                }
            }
        } else {
            $statusProduct = $helper->getIsProductApproval() ?
            SellerProduct::STATUS_DISABLED : SellerProduct::STATUS_ENABLED;
        }

        $status = isset($wholedata['status'])?$wholedata['status']:$statusProduct;
        ...
        */
        ...
        if (isset($type) && $type == SellerProduct::PRODUCT_TYPE_CONFIGURABLE) {
            $allowedtypes = explode(',', $helper->getAllowedProductType());
