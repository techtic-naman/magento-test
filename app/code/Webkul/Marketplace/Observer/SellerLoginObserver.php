    ...
    public function execute(\Magento\Framework\Event\Observer $observer)
    {
        if ($this->_helper->getIsSeparatePanel()) {
            ...
            if ($controller->getRequest()->getPost('vendor_login')) {
                if ($this->_helper->getCustomerAccountRedirect()) {
                    $url = $this->_urlBuilder->getUrl("marketplace/account/dashboard");
                } else {
                    $url = $this->_session->getBeforeAuthUrl() ? $this->_session->getBeforeAuthUrl()
                    :$this->_urlBuilder->getUrl("marketplace/account/dashboard");
                }
