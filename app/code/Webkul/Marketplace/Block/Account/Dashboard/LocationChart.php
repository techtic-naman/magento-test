...
class LocationChart extends \Magento\Framework\View\Element\Template
{
    ...
    public function getYearlySaleLocation()
    {
        ...
        $data = [];
        $curryear = date('Y');
        $dateFrom = $curryear.'-01-01 00:00:00';
        $dateTo = $curryear.'-12-31 23:59:59';
    ...
    public function getMonthlySaleLocation()
    {
        ...
        $currMonth = date('m');
        $currDay = date('d');
        $dateFrom = $curryear.'-'.$currMonth.'-01 00:00:00';
        $dateTo = $curryear.'-'.$currMonth.'-'.$currDay.' 23:59:59';
        $sellerOrderCollection = $this->saleslistFactory->create()
                                  ->getCollection()
                                  ->addFieldToFilter(
                                      'main_table.seller_id',
    ...
    public function getWeeklySaleLocation()
    {
        ...
        $dateFrom = $fromYear.'-'.$fromMonth.'-'.$currWeekStartDay.' 00:00:00';
        $dateTo = $curryear.'-'.$currMonth.'-'.$currWeekEndDay.' 23:59:59';
        $sellerOrderCollection = $this->saleslistFactory->create()
                                ->getCollection()
                                ->addFieldToFilter(
                                    'main_table.seller_id',
    ...
    public function getDailySaleLocation()
    {
        ...
        $currMonth = date('m');
        $currDay = date('d');
        $dateFrom = $curryear.'-'.$currMonth.'-'.$currDay.' 00:00:00';
        $dateTo = $curryear.'-'.$currMonth.'-'.$currDay.' 23:59:59';
        $sellerOrderCollection = $this->saleslistFactory->create()
                                  ->getCollection()
                                  ->addFieldToFilter(
     ...
     * Get array data
     *
     * @param \Magento\Sales\Model\OrderFactory $collection
    ...
    public function getSellerStatisticsGraphUrl($dateType)
    {
        ...
        foreach ($countryRegionArr as $key => $value) {
            foreach ($value as $key2 => $value2) {
                ...
                $chmVal = 'f'.$value2.':Orders-'.$count.' Sales-'.$amount.',000000,0,'.$index.',10';
